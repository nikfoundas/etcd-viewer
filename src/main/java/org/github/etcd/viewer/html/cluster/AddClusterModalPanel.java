...
public class AddClusterModalPanel extends GenericModalPanel<Void> {
    ...
    public AddClusterModalPanel(String id) {
        ...
        form.add(new FormGroupBorder("apiVersionGroup", new ResourceModel("apiVersion.label", "API version"))
                .add(new DropDownChoice<>("apiVersion", Arrays.asList(ApiVersion.values())).setRequired(true)));
...
//                System.out.println("ON SUBMIT -- ADD CLUSTER");
                ...
                EtcdCluster cluster = clusterManager.addCluster(name, address, apiVersion);
